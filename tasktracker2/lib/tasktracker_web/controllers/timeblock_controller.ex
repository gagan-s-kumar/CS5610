defmodule TasktrackerWeb.TimeblockController do
  ...
  def create(conn, %{"timeblock" => timeblock_params}) do
    with {:ok, %Timeblock{} = timeblock} <- Tracker.create_timeblock(timeblock_params) do
      conn
      |> put_status(:created)
  ...
  def show(conn, %{"id" => id}) do
    timeblock = Tracker.get_timeblock!(id)
  ...
  def update(conn, %{"id" => id, "timeblock" => timeblock_params}) do
    ...
    with {:ok, %Timeblock{} = timeblock} <- Tracker.update_timeblock(timeblock, timeblock_params) do
      render(conn, "show.json", timeblock: timeblock)
    end
  end
  ...
  def delete(conn, %{"id" => id}) do
    timeblock = Tracker.get_timeblock!(id)
    with {:ok, %Timeblock{}} <- Tracker.delete_timeblock(timeblock) do
